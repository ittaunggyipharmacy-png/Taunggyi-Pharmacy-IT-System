...
import { 
  ShieldOff, LayoutDashboard, Ticket as TicketIcon, Monitor, 
  ShieldCheck, Megaphone, HardDrive, Settings, HelpCircle, 
  Activity, Users, FileText, Briefcase, Calendar, FolderClock, 
  Printer
} from 'lucide-react';
...
import { 
  Status, ITTicket, ITAsset, BackupLog, BackupSchedule, ContentPlan, CCTVRequest,
  RenewalRecord, PurchaseRecord, SystemSettings, ActivityEntry, TaskEvidence, DailyLog,
  EmployeeProfile, UserRole
} from './types';
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      const unsubSync = subscribeToSync({
        ...
        onRenewals: (updatedRenewals) => setRenewals(updatedRenewals)
      });
  ...
  useEffect(() => {
    if (authReady && currentUser && !isAdmin) {
      ...
      if (!allowedTabs.includes(activeTab) && allowedTabs.length > 0) {
        // we already handle this in the other use effect, but leaving for legacy behavior compatibility
      }
    }
