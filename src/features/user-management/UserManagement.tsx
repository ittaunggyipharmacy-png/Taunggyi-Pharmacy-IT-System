...
export const UserManagement = ({ isSuperAdmin }: { isSuperAdmin: boolean }) => {
 ...
 <div>
 <h2 className="text-2xl font-medium text-slate-800 dark:text-slate-100 tracking-tight">Personnel Access Control</h2>
 <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Assign roles and manage system permissions for authorized staff.</p>
 </div>
 <div className="flex items-center gap-2 bg-indigo-50 px-4 py-2 rounded-xl text-indigo-700 text-xs font-medium text-slate-500 dark:text-slate-400 border border-indigo-100">
 ...
 layout
 key={user.uid}
 className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row md:items-center gap-6 shadow-sm hover:shadow-md transition-shadow"
 >
 ...
 <label className="text-xs font-medium text-slate-400 ml-1">Access Level</label>
 <select
 disabled={updating === user.uid || !isSuperAdmin}
 value={user.role}
 onChange={(e) => handleRoleChange(user.uid, e.target.value as UserRole)}
 className={`px-4 py-2.5 rounded-xl text-xs font-medium outline-none border transition-all appearance-none cursor-pointer bg-slate-50 ${updating === user.uid || !isSuperAdmin ? 'opacity-50 cursor-not-allowed' : 'hover:border-indigo-300 focus:ring-4 focus:ring-indigo-50 border-slate-200 dark:border-slate-800'}`}
 >
 ...
 <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-600 rounded-full text-xs font-medium border border-emerald-100">
 ...
 {users.length === 0 && (
 <div className="text-center py-20 bg-slate-50 dark:bg-slate-800/50 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
 <UserCog size={40} className="mx-auto text-slate-300 mb-3" />
 <p className="text-sm font-medium text-slate-400 ">No system users found</p>
 </div>
 )}
 </div>
 </div>
