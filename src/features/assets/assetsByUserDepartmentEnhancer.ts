...
type AssetDepartment = { id: string; department: string };

const normalize = (value: unknown) => String(value ?? '').trim().toLowerCase();

const loadAssetDepartments = async (): Promise<AssetDepartment[]> => {
  const { data, error } = await supabase.from('assets').select('id,department');
  if (error) throw error;
  return (data || [])
    .filter((asset: any) => asset?.id && String(asset.department || '').trim())
    .map((asset: any) => ({ id: String(asset.id), department: String(asset.department).trim() }));
};
...
const installDepartmentFilter = async (panel: HTMLElement) => {
  ...
  const assetDepartmentById = new Map(assets.map(asset => [asset.id, normalize(asset.department)]));
  const control = document.createElement('div');
  control.className = 'mt-3 flex items-center gap-2 sm:mt-0';
  ...
  const applyFilter = () => {
    ...
    Array.from(list.children).forEach(row => {
      ...
      const assetCode = element.querySelector('span.font-semibold')?.textContent?.trim() || '';
      const matchingAsset = assets.find(asset => asset.id === assetCode);
      const department = matchingAsset ? normalize(matchingAsset.department) : (() => {
        const text = normalize(element.textContent);
        const byDepartment = departments.find(item => text.includes(normalize(item)));
        return normalize(byDepartment);
      })();
