...
import { ArrowLeft, Building2, CalendarDays, ChevronRight, Package, Plus, Search, UserRound, X, UserPlus } from 'lucide-react';
...
export function AssetUsersPage({ currentUserId, isAdmin }: AssetUsersPageProps) {
  ...
  const holders = useMemo<AssetHolder[]>(() => {
    ...
    const peopleHolders: AssetHolder[] = people
      .filter(person => !person.linkedUserId || !loginIds.has(person.linkedUserId))
  ...
  const handleAssign = async () => {
    if (!selectedHolder || !assetToAssign || selectedHolder.kind !== 'person') {
      if (selectedHolder?.kind === 'login' && assetToAssign) {
        toast.error('Login User ကိုလည်း assign လုပ်လို့ရပေမယ့် ဒီနေရာမှာ Asset Person record ကို အရင်ဖန်တီးထားတဲ့ workflow ကို သုံးပါ။');
      }
      return;
    }
    setAssigning(true);
    try {
      ...
      toast.success(`${selectedHolder.person.fullName} ဆီ Asset assign လုပ်ပြီးပါပြီ။`);
      setAssetToAssign('');
      setAssignNotes('');
  ...
  if (selectedHolder) {
    const info = selectedHolder.kind === 'person' ? selectedHolder.person : selectedHolder.user;
    ...
    return <section className="space-y-5">
      <button type="button" onClick={() => setSelectedHolder(null)} className="inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-blue-600"><ArrowLeft size={17} /> Back to Asset Users</button>
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-600"><UserRound size={25} /></div><div><div className="flex items-center gap-2"><h1 className="text-xl font-bold text-slate-900">{holderName}</h1><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-600">{isAssetPerson ? 'ASSET USER' : 'LOGIN USER'}</span></div><p className="text-sm text-slate-500">{position || '-'} · {department || '-'} · {branch || '-'}</p><p className="mt-1 text-xs text-slate-400">Employee ID: {employeeId || '-'}{selectedHolder.kind === 'login' ? ` · ${selectedHolder.user.email}` : ''}</p></div></div>
      ...
      {!isAssetPerson && <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-800">ဒီ Login User ကို Asset User အဖြစ် automatically link လုပ်နိုင်တဲ့ architecture ပါပြီးသားပါ။ Non-login employee အတွက်တော့ Asset User record သီးသန့်သုံးပါ။</div>}
