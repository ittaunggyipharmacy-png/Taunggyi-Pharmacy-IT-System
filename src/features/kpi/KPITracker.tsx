...
export const KPITracker: React.FC<{ userRole?: UserRole }> = ({ userRole }) => {
 ...
 const [currentUser, setCurrentUser] = useState<{ uid: string; id: string; displayName?: string; email?: string } | null>(null);
 ...
 <div>
 <h2 className="text-2xl font-medium text-slate-800 dark:text-slate-100 tracking-tight flex items-center gap-2">
 <ClipboardList className="text-indigo-600" />
 Operational KPI Tracker
 </h2>
 <p className="text-sm text-slate-500 dark:text-slate-400">Taunggyi Pharmacy Excellence Monitoring</p>
 </div>
 ...
 isSelected ? "bg-white dark:bg-slate-900/20" : "bg-slate-100 text-slate-500 dark:text-slate-400"
 }`}>
 {progress}%
 </span>
 </button>
 ...
 <h3 className="text-sm font-medium opacity-80 ">Overall Progress</h3>
 <div className="mt-2 text-4xl font-medium">{calculateProgress()}%</div>
 <p className="mt-2 text-xs font-medium opacity-70">Based on {currentTasks.length} {view} objectives</p>
 </div>
 </div>

 {/* Task List */}
 <div className="lg:col-span-3">
 <div className="enterprise-card overflow-hidden bg-white dark:bg-slate-900">
 <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
 <h3 className="font-medium text-slate-800 dark:text-slate-100 flex items-center gap-2  text-sm">
 ...
 <p className="text-xs text-slate-400 font-medium text-slate-500 dark:text-slate-400">Syncing Data...</p>
 </div>
 ) : (
 ...
 isCompleted ? "border-emerald-500 text-emerald-500" : "border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400"
 }`}>
 <Camera size={16} />
 </div>
 </div>
 ...
 className="p-1.5 bg-slate-100 text-slate-500 dark:text-slate-400 rounded-lg hover:bg-slate-200 transition-colors"
 ...
 <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${CATEGORY_COLORS[task.category]}`}>
 {task.category}
 </span>
 {task.dayOfWeek !== undefined && (
 <span className={`text-xs font-medium px-2 py-0.5 rounded-full flex items-center gap-1 ${
 isDueToday ? "bg-rose-100 text-rose-700 border-rose-200" : "bg-slate-100 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-800"
 }`}>
 <Clock size={10} />
 ...
 {task.frequency && (
 <span className="text-xs font-medium px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-800 bg-slate-50 text-slate-600 dark:text-slate-300">
 {task.frequency}
 </span>
 )}
 {typeof value === 'string' && (
 <span className="text-xs font-medium text-slate-400 bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100">
 ...
 {task.maxCount && (
 <div className="text-xs font-medium text-slate-500 dark:text-slate-400 bg-slate-100 px-3 py-1 rounded-full">
 {Number(value) || 0} / {task.maxCount}
 </div>
 )}
 {isDueToday && !isCompleted && (
 <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-indigo-600 text-white animate-pulse">
 DUE TODAY
 </span>
 )}
 </div>
 ...
 className="p-1 hover:bg-white dark:bg-slate-900 rounded-lg text-slate-400 disabled:opacity-30"
 disabled={!value}
 >
 <Minus size={14} />
 </button>
 ...
 className="w-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded text-center font-medium text-slate-800 dark:text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
 />
 ) : (
 <span className="w-12 text-center font-medium text-slate-800 dark:text-slate-100">{value || 0} / {task.maxCount}</span>
 )}
 
 <span className="text-xs font-medium text-slate-400 mx-1">/ {task.maxCount}</span>
 ...
 <p className={cn("text-base font-semibold", isCompleted ? "text-slate-400 line-through" : "text-slate-700 dark:text-slate-200")}>{task.text}</p>
 <span className={cn("text-xs font-medium px-2 py-0.5 rounded-full border", CATEGORY_COLORS[task.category])}>{task.category} (Custom)</span>
 </div>
 </div>
 );
 })}
 ...
 className="text-xs font-medium text-indigo-600 flex items-center gap-2"
 >
 ...
 >
 <h3 className="text-xl font-medium text-slate-800 dark:text-slate-100 mb-2">Upload Photo Evidence</h3>
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
 Please provide a URL for the photo proof for: <span className="font-medium text-indigo-600">{TASKS.find(t => t.id === showEvidenceModal)?.text}</span>
 </p>
 
 <div className="space-y-4">
 <div>
 <label className="block text-xs font-medium text-slate-400 mb-1.5 ml-1">Image URL</label>
 <input 
 type="url"
 value={evidenceUrl}
 onChange={e => setEvidenceUrl(e.target.value)}
 placeholder="https://example.com/photo.jpg"
 className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-indigo-500/10 transition-all font-sans"
 />
 </div>
 ...
 <button 
 onClick={() => setShowEvidenceModal(null)}
 className="flex-1 px-4 py-3.5 bg-slate-100 text-slate-600 dark:text-slate-300 rounded-2xl font-medium text-xs hover:bg-slate-200 transition-all"
 >
 Cancel
 </button>
 <button 
 ...
 className="flex-1 px-4 py-3.5 bg-indigo-600 text-white rounded-2xl font-medium text-xs hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-100 disabled:opacity-50"
 >
 Confirm Upload
 </button>
 </div>
 ...
 className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-8"
 >
 <h3 className="text-xl font-medium text-slate-800 dark:text-slate-100 mb-2">Add Custom Task</h3>
 <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
 Add a new task for <span className="font-medium text-indigo-600">{isAddingCustomTask}</span>
 </p>
 
 <div className="space-y-4">
 <div>
 <label className="block text-xs font-medium text-slate-400 mb-1.5 ml-1">Task Description</label>
 <input 
 type="text"
