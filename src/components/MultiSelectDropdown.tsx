  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef}>
      ...
      <button 
        ...
        className={cn(
          "w-full flex items-center justify-between px-3 py-2 bg-white border border-slate-200 rounded-xl text-[11px] font-bold transition-all text-slate-700 hover:border-indigo-400 focus:ring-2 focus:ring-indigo-500/10 outline-none",
        ...
        {isOpen && (
          ...
          >
            <div className="p-2 border-b border-slate-100">
               <div className="relative">
                 ...
                 <input 
                   ...
                   className="w-full pl-8 pr-3 py-2 bg-slate-50 border-none rounded-lg text-xs outline-none focus:ring-1 focus:ring-indigo-500/10"
                   placeholder="Search..."
                 />
               </div>
            </div>
            ...
            <div className="px-2 py-1 border-b border-slate-100">
               <button 
                  onClick={toggleAll}
                  className="w-full text-left px-2 py-1.5 text-xs text-indigo-600 font-bold hover:bg-slate-50 rounded"
               >
            ...
            <div className="max-h-60 overflow-y-auto py-1 custom-scrollbar">
              {filteredOptions.length === 0 && <div className="px-4 py-3 text-[10px] text-slate-400 italic text-center">No options</div>}
              {filteredOptions.map((opt) => (
                <button
                  ...
                  className={cn(
                    ...
                    selected.includes(opt) ? "bg-indigo-50 text-slate-900 font-bold" : "text-slate-600 hover:bg-slate-50"
                  )}
                >
