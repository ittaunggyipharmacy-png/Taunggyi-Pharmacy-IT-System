...
import { saveMeetingMinute, deleteMeetingMinute, saveActivity } from "../services/firestoreService";
...
export default function MeetingMinutesModule({ userRole, isAdmin }: MeetingMinutesModuleProps) {
  ...
  // Subscribe to real-time Meeting Minutes & Users
  useEffect(() => {
    setLoading(true);
    // 1. Subscribe to meeting minutes ordered by date descending
    const meetingsQuery = query(collection(db, "meeting_minutes"), orderBy("date", "desc"));
    const unsubMeetings = onSnapshot(meetingsQuery, (snapshot) => {
      const list = snapshot.docs.map(doc => ({
        ...doc.data() as MeetingMinute,
        id: doc.id
      }));
      setMeetings(list);
      setLoading(false);
    ...
    const unsubUsers = onSnapshot(collection(db, "app_users"), (snapshot) => {
      setUsers(snapshot.docs.map(doc => doc.data() as SystemUser));
    });
  ...
  const getDueBadgeClass = (dueDateStr: string, status: string) => {
    ...
    try {
      ...
      if (isToday(due)) {
        return "bg-amber-100 text-amber-800 border-amber-200 animate-pulse font-bold";
      }
  ...
  return (
    <div className="enterprise-container p-4 lg:p-8 flex flex-col gap-6 max-h-[92vh] overflow-y-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0">
        <div>
          <h2 className="text-xl lg:text-2xl font-black text-slate-800 tracking-tight flex items-center gap-3">
            ...
            MEETING MINUTES & FOLLOW-UP ACTION ITEMS
          </h2>
        ...
        <button
          ...
          className="px-5 py-3 bg-indigo-600 text-white font-bold rounded-xl uppercase text-[11px] tracking-widest hover:bg-indigo-500 transition-all flex items-center justify-center gap-2 shadow-lg hover:translate-y-[-1px] self-start lg:self-auto"
        >
          <Plus size={16} />
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-sm shrink-0">
        <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-xl self-start">
          <button
            ...
            className={cn(
              ...
              activeSubTab === "minutes" ? "bg-white text-slate-800 shadow" : "text-slate-500 hover:text-slate-800"
            )}
          >
          ...
          <button
            ...
            className={cn(
              "px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all flex items-center gap-1.5",
              activeSubTab === "followup" ? "bg-white text-slate-800 shadow animate-pulse" : "text-slate-500 hover:text-slate-800"
            )}
          >
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-indigo-500 transition-colors"
          />
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-slate-400">
          ...
          <p className="text-xs font-bold tracking-widest uppercase">Fetching minutes from database...</p>
        </div>
      ) : activeSubTab === "minutes" ? (
        ...
        filteredMeetings.length === 0 ? (
          <div className="enterprise-card p-16 text-center text-slate-300 border-2 border-dashed border-slate-100 flex flex-col items-center justify-center gap-3 rounded-2xl">
            ...
            <p className="font-bold text-slate-400">အစည်းအဝေးမှတ်တမ်း မရှိပါ။</p>
            <button
              onClick={() => handleOpenCreateModal(null)}
              className="mt-2 text-xs text-indigo-500 border border-indigo-200 hover:bg-slate-50 rounded-xl px-4 py-2 transition-colors font-bold uppercase tracking-wider"
            >
            ...
            {filteredMeetings.map((m) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-md cursor-pointer hover:border-indigo-100 transition-all flex flex-col justify-between"
                >
                  <div>
                    ...
                    <h3 className="text-base font-bold text-slate-800 line-clamp-2 mb-2 tracking-tight uppercase hover:text-indigo-600 transition-colors">
                      {m.title}
                    </h3>
                  ...
                  <div className="pt-4 border-t border-slate-50 mt-4">
                    <div className="flex items-center justify-between text-xs text-slate-500 mb-2 font-bold uppercase">
                      <span className="text-[10px] tracking-widest font-black text-slate-400">Action items status</span>
                      <span className="text-[10px] tracking-wide text-indigo-600">
                    ...
                    {m.attendees?.length > 0 && (
                      <div className="flex items-center gap-1 overflow-hidden whitespace-nowrap text-ellipsis mt-3">
                        ...
                        <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider overflow-hidden text-ellipsis">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {/* Filter by Status */}
              <div>
                <label className="text-[10px] text-slate-400 font-black tracking-widest uppercase mb-1.5 block">Status</label>
                <select
                  value={statusFilter}
                  onChange={(e) => setStatusFilter(e.target.value)}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:border-indigo-500 font-bold text-slate-700"
                >
              ...
              <div>
                <label className="text-[10px] text-slate-400 font-black tracking-widest uppercase mb-1.5 block">Department / Task Scope</label>
                <select
                  value={deptFilter}
              ...
              {/* Filter by Assignee */}
              <div>
                <label className="text-[10px] text-slate-400 font-black tracking-widest uppercase mb-1.5 block">Assignee (လုပ်ငန်းတာဝန်ကျသူ)</label>
                <select
          ...
          {allFollowUpItems.length === 0 ? (
            <div className="bg-white border border-slate-100 p-16 text-center text-slate-300 rounded-3xl border-2 border-dashed flex flex-col items-center justify-center gap-3">
              <ClipboardList size={40} className="text-slate-400" />
              <p className="font-bold text-slate-400">ရှာဖွေမှုနှင့် ကိုက်ညီသော Follow-up တာဝန်များ မရှိပါ။</p>
            </div>
          ) : (
                ...
                <table className="w-full text-left text-sm whitespace-nowrap border-collapse">
                  <thead>
                    <tr className="bg-slate-50 text-[10px] text-slate-400 font-black uppercase tracking-widest border-b border-slate-100">
                      <th className="px-6 py-4">Action Item (လုပ်ငန်းတာဝန်)</th>
                      <th className="px-6 py-4">Assigned To (တာဝန်ကျသူ)</th>
                      <th className="px-6 py-4">Scope / Goal</th>
                      <th className="px-6 py-4">Due Target (သတ်မှတ်ရက်)</th>
                      <th className="px-6 py-4 text-center">Current Status (အခြေအနေ)</th>
                      <th className="px-6 py-4">Remarks / Follow-up Updates (တိုးတက်မှုမှတ်တမ်း)</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="px-6 py-4 max-w-sm">
                          <div className="flex flex-col gap-1">
                            <span className="font-bold text-slate-800 break-words whitespace-pre-wrap text-xs lg:text-sm">
                              {item.task}
                            </span>
                            <span className="text-[10px] text-slate-400 font-medium flex items-center gap-1">
                              <FileText size={10} />
                              Meeting: <strong className="hover:underline cursor-pointer text-slate-500" onClick={() => setSelectedMeeting(meeting)}>{meeting.title}</strong>
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          {item.department && (
                            <span className="px-2.5 py-1 text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-600 rounded-lg border border-slate-200">
                              {item.department}
                            </span>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            <span className={cn(
                              "px-2 py-0.5 text-[9px] font-black uppercase tracking-widest rounded-md mt-1 border border-solid self-start",
                        ...
                        <td className="px-6 py-4 text-center">
                          <select
                            ...
                            className={cn(
                              "px-3 py-1.5 rounded-xl text-xs font-black uppercase tracking-wider border text-center outline-none cursor-pointer focus:ring-1 focus:ring-indigo-500",
                          ...
                          {editingRemarkItemId === item.id ? (
                            <div className="flex items-center gap-2 max-w-xs">
                              <input
                                ...
                                className="px-2 py-1.5 min-w-[200px] border border-slate-200 rounded-lg text-xs outline-none shadow-inner"
                              />
                              <button
                                onClick={() => handleSaveSingleActionItemRemark(meeting, item.id)}
                                className="p-1 px-2.5 bg-emerald-600 text-white rounded-lg text-xs font-bold uppercase tracking-widest"
                              >
                                Save
                              </button>
                              <button
                                onClick={() => setEditingRemarkItemId(null)}
                                className="p-1 px-2.5 bg-slate-200 text-slate-600 rounded-lg text-xs font-bold uppercase tracking-widest"
                              >
                                Cancel
                              </button>
                            </div>
                            ...
                            <div className="flex items-center gap-2 group cursor-pointer max-w-sm" onClick={() => {
                              ...
                              <span className={cn(
                                ...
                                item.remarks ? "text-slate-600 font-medium" : "text-slate-350"
                              )}>
        ...
        {selectedMeeting && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-xs">
            <motion.div
              ...
              className="bg-white p-0 w-full h-full sm:h-auto sm:max-w-3xl rounded-none sm:rounded-3xl overflow-hidden flex flex-col sm:max-h-[88vh] shadow-2xl relative"
            >
              {/* Top Banner details */}
              <div className="p-6 sm:p-8 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
                <div className="flex-1 mr-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest">Meeting Minutes Detail</span>
                    <span className="px-2 py-0.5 text-[9px] font-black bg-indigo-50 border border-indigo-100 text-indigo-600 uppercase tracking-widest rounded-md">
                      Real-time
                    </span>
                  </div>
                  <h3 className="text-lg sm:text-xl font-bold text-slate-800 tracking-tight break-words whitespace-pre-wrap uppercase">
              ...
              <div className="overflow-y-auto p-6 sm:p-8 flex-1 space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-xs">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-slate-700 flex items-center gap-1 truncate">
                      <MapPin size={12} className="text-slate-400" />
                      {selectedMeeting.location || "-"}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Recorded By</p>
                    <p className="font-bold text-slate-700 flex items-center gap-1 truncate text-indigo-600">
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Meeting Minutes Summary & Details</h4>
                  <div className="p-6 bg-slate-50 border border-slate-200 rounded-3xl text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">
                ...
                {selectedMeeting.attendees?.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Attendees (တက်ရောက်သူများ)</h4>
                    <div className="flex flex-wrap gap-2">
                      {selectedMeeting.attendees.map((attendee, idx) => (
                        <span key={idx} className="px-3 py-1.5 bg-indigo-50 border border-indigo-100 text-indigo-700 font-bold rounded-full text-xs flex items-center gap-1">
                          <User size={10} />
                          {attendee}
                        </span>
                      ))}
                  ...
                  {(!selectedMeeting.actionItems || selectedMeeting.actionItems.length === 0) ? (
                    ...
                    <div className="space-y-4">
                      {selectedMeeting.actionItems.map((item) => (
                        <div key={item.id} className="p-4 bg-white border border-slate-150 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xs">
                          <div className="flex-1 space-y-1">
                            <p className="font-bold text-slate-800 text-sm whitespace-pre-wrap">{item.task}</p>
                            ...
                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-500 pt-1">
                              ...
                              {item.department && (
                                <span className="px-2 py-0.5 text-[9px] bg-slate-100 border border-slate-250 text-slate-500 font-bold rounded uppercase">
                                  {item.department}
                                </span>
                              )}
                            ...
                            <p className={cn(
                              "text-[10px] font-black tracking-wider uppercase inline-block px-1.5 py-0.5 rounded",
                              getDueBadgeClass(item.dueDate, item.status)
                            )}>
                              {getDueText(item.dueDate, item.status)}
                            </p>
                          ...
                          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 max-w-sm">
                            <select
                              ...
                              className={cn(
                                "px-3 py-1.5 rounded-xl text-xs font-black uppercase border tracking-wider outline-none text-center cursor-pointer",
                                getStatusBadgeClass(item.status)
                              )}
                            >
                              <option value="Pending">Pending</option>
                            ...
                            {editingRemarkItemId === item.id ? (
                              <div className="flex items-center gap-2 shrink-0">
                                <input
                                  ...
                                  value={tempRemark}
                                  onChange={(e) => setTempRemark(e.target.value)}
                                  className="px-2 py-1 bg-slate-50 border rounded text-xs"
                                  placeholder="Update remark..."
                                />
                                <button
                                  onClick={() => handleSaveSingleActionItemRemark(selectedMeeting, item.id)}
                                  className="p-1 px-2.5 bg-indigo-600 text-white rounded font-bold uppercase text-[9px] tracking-widest"
                                >
                                  Save
                                </button>
                                <button
                                  onClick={() => { setEditingRemarkItemId(null); setTempRemark(""); }}
                                  className="p-1 px-2.5 bg-slate-200 text-slate-600 rounded font-bold uppercase text-[9px] tracking-widest"
                                >
                                  Cancel
                                </button>
                              </div>
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 border border-slate-200 rounded-xl text-xs font-bold bg-slate-50 hover:bg-slate-100 text-slate-600 transition-colors flex items-center justify-center gap-1.5"
                              >
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-xs">
            <motion.div
              ...
              exit={{ opacity: 0, y: 30 }}
              className="bg-white p-0 w-full h-full sm:h-auto sm:max-w-4xl rounded-none sm:rounded-3xl overflow-hidden flex flex-col sm:max-h-[92vh] shadow-2xl"
            >
              {/* Form Title Banner */}
              <div className="p-6 sm:p-8 border-b border-slate-100 bg-white flex items-center justify-between shrink-0">
                <div>
                  <h3 className="text-lg sm:text-xl font-bold text-slate-800 tracking-tight uppercase">
              ...
              <form onSubmit={handleSaveMeeting} className="overflow-y-auto p-6 sm:p-8 flex-1 space-y-6">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] md:text-xs text-slate-400 font-black uppercase tracking-widest">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm transition-colors focus:border-indigo-500 outline-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none transition-colors"
                    />
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none placeholder-slate-350"
                    />
                  </div>
                ...
                <div className="space-y-2.5">
                  <label className="text-[10px] md:text-xs text-slate-400 font-black uppercase tracking-widest block">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none"
                    />
                    <button
                      type="button"
                      onClick={handleAddAttendee}
                      className="px-6 py-3 bg-indigo-600 text-white rounded-2xl text-xs font-bold uppercase tracking-widest"
                    >
                      Add
                    </button>
                  </div>
                  ...
                  {formAttendees.length > 0 && (
                    <div className="flex flex-wrap gap-2 p-3 bg-slate-50 border rounded-2xl">
                      {formAttendees.map((att, index) => (
                        <span key={index} className="px-3 py-1 bg-white border border-slate-200 text-slate-700 font-bold rounded-full text-xs flex items-center gap-1.5">
                          {att}
                          <button
                            type="button"
                            onClick={() => handleRemoveAttendee(index)}
                            className="text-slate-400 hover:text-rose-500 font-black text-xs shrink-0"
                          >
                            &times;
                          </button>
                        </span>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] md:text-xs text-slate-400 font-black uppercase tracking-widest text-slate-500">
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-sans whitespace-pre-wrap outline-none focus:border-indigo-500 focus:bg-white resize-y"
                  ></textarea>
                </div>
                ...
                <div className="space-y-4 pt-6 border-t border-slate-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[9px] text-slate-400 font-semibold tracking-wide block mt-0.5">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors flex items-center gap-1"
                    >
                      <Plus size={14} />
                      Add Task
                    </button>
                  ...
                  {formActionItems.length === 0 ? (
                    <div className="p-8 text-center bg-slate-50 border-2 border-dashed border-slate-150 text-slate-400 text-sm rounded-2xl italic">
                      အစည်းအဝေးမှ တာဝန်ပေးချက်များ မရှိသေးပါ (Add Task ကိုနှိပ်၍ သတ်မှတ်ပါ)။
                    </div>
                  ) : (
                    <div className="space-y-4 max-h-[40vh] overflow-y-auto pr-2">
                      {formActionItems.map((item, idx) => (
                        <div key={item.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl relative flex flex-col md:flex-row gap-4 items-stretch md:items-end">
                          ...
                          {/* Task Descr */}
                          <div className="flex-1 space-y-1">
                            <label className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Task / Action Required *</label>
                            <input
                              type="text"
                              required
                          ...
                          <div className="w-full md:w-1/4 space-y-1">
                            <label className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Assigned Staff</label>
                            <input
                              type="text"
                              list="staff-list-mt"
                          ...
                          <div className="w-full md:w-1/5 space-y-1">
                            <label className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Scope / Department</label>
                            <select
                          ...
                          {/* Due date picker */}
                          <div className="w-full md:w-1/5 space-y-1">
                            <label className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block">Target Due Date</label>
                            <input
                              type="date"
                              value={item.dueDate}
                ...
                <div className="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                  <button
                    type="button"
                    onClick={() => setIsCreateModalOpen(false)}
                    className="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold uppercase tracking-widest transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-all shadow-md"
                  >
...
function mt_analytics_cards(metrics: { totalMeetings: number; pending: number; inProgress: number; completed: number }) {
  const data = [
    { title: "Total Meetings", val: metrics.totalMeetings, desc: "Recorded अစည်းအဝေးများ", color: "border-slate-100 text-slate-800" },
    { title: "Pending Actions", val: metrics.pending, desc: "လုပ်ရန် ကျန်ရှိသည်", color: "border-slate-100 text-slate-500" },
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
      {data.map((item, idx) => (
        <div key={idx} className={cn("bg-white border rounded-3xl p-5 shadow-xs flex flex-col justify-between", item.color)}>
          <span className="text-[10px] text-slate-400 font-black tracking-widest uppercase">{item.title}</span>
